...
Command-line interface for CLI-DeepResearch — a LangGraph multi-agent research system.
"""
...
def print_welcome() -> None:
    """打印欢迎界面"""
    console.print("\n")
    print_header(f"CLI-DeepResearch v{__version__}")
    console.print("[yellow]欢迎使用基于 LangGraph 的多智能体深度研究系统！[/yellow]")
        ...
        while True:
            try:
                ...
                if choice == "1":
                    ...
                    # 退出程序
                    console.print("\n[yellow]感谢使用 CLI-DeepResearch！再见！[/yellow]\n")
                    return 0
...
def parse_args(argv: Any) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="deepresearch",
        description="CLI-DeepResearch — 基于 LangGraph 的多智能体深度研究系统"
    )
    parser.add_argument(
        "query",
        nargs="?",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"CLI-DeepResearch {__version__}"
    )
